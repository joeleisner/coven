...
test('grimoire(element) creates a per-element store', () => {
	const el = document.createElement('div') as GrimoireElement;
	const store = grimoire(el);
	assert(typeof store === 'object');
});

test('grimoire(element) returns the same store on repeat calls', () => {
	const el = document.createElement('div') as GrimoireElement;
	assertStrictEquals(grimoire(el), grimoire(el));
});

test('grimoire(element, type) creates a typed slot', () => {
	const el = document.createElement('div') as GrimoireElement;
...
test('different slot symbols do not collide', () => {
	const el = document.createElement('div') as GrimoireElement;
...
test('different elements have isolated grimoires', () => {
...
test('grimoire.shared(element, type) stores on the constructor', () => {
...
test('grimoire.shared is isolated between classes', () => {
...
test('grimoire and grimoire.shared do not collide on the same symbol', () => {
